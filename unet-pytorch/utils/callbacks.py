...
class LossHistory():
    """Record and visualize training/validation loss history"""
    ...
    def append_loss(self, epoch, loss, val_loss=None):
        """Record loss values for each epoch"""
...
class EvalCallback():
    """Callback for evaluating model performance during training"""
    ...
    def get_miou_png(self, image):
        ...
        # The code only supports RGB images - others will be converted
        image       = cvtColor(image)
        original_h  = np.array(image).shape[0]
        original_w  = np.array(image).shape[1]
        
        # Add gray bars to maintain aspect ratio during resize
        # Alternatively could resize directly without preserving aspect ratio
    ...
    def on_epoch_end(self, epoch, model_eval):
        """Calculate and record mIoU at end of each evaluation period"""
        if epoch % self.period == 0 and self.eval_flag:
            ...
            print("mIoU calculation complete.")
