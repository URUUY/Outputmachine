...
#   into one file, with mode switching via the 'mode' parameter
#----------------------------------------------------#
import time
import os
import cv2
...
if __name__ == "__main__":
    #-------------------------------------------------------------------------#
    #   To modify colors for different classes, change self.colors in __init__
    #-------------------------------------------------------------------------#
    deeplab = DeeplabV3()
    #----------------------------------------------------------------------------------------------------------#
    #   mode specifies the operation mode:
    #   'predict'       Single image prediction (see detailed comments below)
    #   'video'         Video detection (camera or video file)
    #   'fps'           FPS testing (uses street.jpg in img folder)
    #   'dir_predict'   Batch prediction on folder (default scans img folder, saves to img_out)
    ...
    #   name_classes    Class names (same as in json_to_dataset) for printing counts
    #
    ...
    if mode == "predict":
        ...
        1. For batch prediction, use os.listdir() to traverse folder and Image.open()
        ...
        try:
            ...
            # Generate save path
            save_path = os.path.join(output_dir, os.path.basename(os.path.splitext(img)[0]) + "_predicted2.png")
